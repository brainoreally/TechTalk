...
void CLProgram::queueKernel(const char* kernel_key, size_t global, size_t local)
{
    ...
    if (err != 0)
        int test = 1;
    // Execute the kernel
    err = clEnqueueNDRangeKernel(command_queue, kernels[kernel_key], 1, NULL, &global, &local, 0, NULL, NULL); //&kernel_event);
...
void CLProgram::setupNetworkOpenCL(NetworkParams* params) {
    ...
    createBuffer<float>("bestLoss", 1); // float used to set learning rate
    createBuffer<float>("learningRate", 1); // float used to set learning rate
    ...
    std::vector<KernelParam> network_kernels = {
        { "network_output", { "networkCounts", "layerSizes", "layerActivations", "correctOutput", "neuronValues", "weights", "biases", "bestLoss" }},
